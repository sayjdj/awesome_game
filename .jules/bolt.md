...
**Learning:** Using `@import url(...)` for Google Fonts in CSS files causes a serial request chain (HTML -> CSS -> Font) which significantly delays Largest Contentful Paint (LCP) and blocks rendering until the font is fetched.
**Action:** Always prefer loading external fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` inside CSS to allow parallel resource downloading.
