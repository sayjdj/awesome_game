...
**Learning:** Using `@import` inside a CSS file to load Google Fonts creates a serial request chain (HTML -> CSS -> Font CSS -> Font File) that delays the first contentful paint and blocks rendering in this legacy frontend setup.
**Action:** Always load external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS files to prevent render-blocking request chains.
