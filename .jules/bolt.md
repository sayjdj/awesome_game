...
## 2024-05-24 - Font Loading via @import
**Learning:** Using `@import` in CSS files creates a sequential, render-blocking request chain (HTML -> CSS -> Font CSS) that significantly delays First Contentful Paint (FCP).
**Action:** Always load external fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` to allow parallel discovery and downloading.
