...
**Learning:** Using `@import` to load external web fonts (like Google Fonts) inside a CSS file creates a render-blocking request chain. The browser must first download and parse the CSS file before discovering the `@import` statement and initiating the download for the font.
**Action:** Always prefer loading external assets like fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>`. This enables parallel downloading and significantly improves First Contentful Paint (FCP).
