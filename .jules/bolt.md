...
## 2024-06-17 - Render-blocking CSS @import
**Learning:** Using `@import` in CSS files to load Google Fonts creates a sequential render-blocking request chain (HTML -> CSS -> Font), which delays First Contentful Paint (FCP) and degrades frontend performance.
**Action:** Always load external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS.
