...
**Learning:** Using `@import` in CSS files for external resources (like Google Fonts) creates a sequential dependency chain (HTML -> CSS -> Font URL). This delays font downloading and blocks rendering, increasing First Contentful Paint (FCP) times.
**Action:** Always load external assets like fonts directly in the HTML `<head>` using `<link rel="preconnect">` and `<link rel="stylesheet">` to allow parallel downloads and avoid request chains.
