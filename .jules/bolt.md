...
## 2024-05-24 - Google Fonts @import Performance
**Learning:** Using `@import` in CSS files to load external assets like Google Fonts creates a render-blocking request chain (HTML -> CSS -> Font). The browser cannot begin downloading the font until the CSS file is fully parsed, delaying the initial paint.
**Action:** Always prefer loading external fonts using `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>`. This allows the browser to discover and download the font in parallel with the CSS, significantly reducing time-to-first-paint.
