...
**Learning:** Loading external fonts (like Google Fonts) using `@import` inside a CSS file creates a serial request chain (HTML -> CSS -> Font CSS -> Font Files), delaying the First Contentful Paint (FCP) and potentially causing layout shifts.
**Action:** Always load external fonts using `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` to enable parallel downloading and faster rendering.
