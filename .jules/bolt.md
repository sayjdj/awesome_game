...
**Learning:** The legacy games in this codebase use `@import` in `style.css` to load Google Fonts, which creates a sequential, render-blocking request chain and delays First Contentful Paint.
**Action:** Always prefer loading external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS.
