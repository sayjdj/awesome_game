...
**Learning:** Using `@import` to load external web fonts (like Google Fonts) in CSS files creates a render-blocking request chain (HTML -> CSS -> Font). The browser cannot begin downloading the font until the CSS file is fully parsed, which delays text rendering.
**Action:** Always load external fonts using `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS to allow parallel downloading.
