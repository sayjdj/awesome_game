...
## 2024-05-25 - CSS @import Render Blocking
**Learning:** Using `@import` in a CSS file to load web fonts (like Google Fonts) creates a sequential request chain (HTML -> CSS -> Font). The browser cannot start downloading the font until the CSS is downloaded and parsed, causing a delay in First Contentful Paint (FCP) and potentially triggering layout shifts or invisible text.
**Action:** Always prefer loading external fonts and stylesheets using `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>`. This allows the browser to discover and download these assets in parallel with the main CSS file, significantly speeding up FCP.
