...
**Learning:** Using `@import` to load fonts inside CSS files creates a sequential request chain (HTML -> CSS -> Font CSS -> Font File) which delays the critical rendering path and increases the First Contentful Paint (FCP) time.
**Action:** Always load external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS files to allow parallel downloading.
