...
## 2024-05-24 - CSS @import Font Loading
**Learning:** Using `@import` in CSS to load Google Fonts creates a render-blocking request chain (HTML -> CSS -> Font CSS -> Font File). This delays the First Contentful Paint (FCP) unnecessarily.
**Action:** Always load external fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of `@import` in CSS files to allow parallel downloading and faster rendering.
