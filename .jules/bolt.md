...
**Learning:** Using `@import` in CSS files creates sequential, render-blocking network requests, significantly delaying the First Contentful Paint (FCP) as the browser must download, parse the CSS, and then fetch the external assets.
**Action:** Always load external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS files.
