...
**Learning:** Using `@import` in CSS for external resources like Google Fonts creates a sequential render-blocking request chain (HTML -> CSS -> Font CSS).
**Action:** Always prefer using `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` for external assets to enable parallel downloading and faster First Contentful Paint.
