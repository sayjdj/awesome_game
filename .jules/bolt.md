...
## 2024-10-27 - Render-blocking CSS @import
**Learning:** Using `@import` within a CSS file to load external fonts creates a sequential request chain (HTML -> CSS -> Font CSS -> Font File) that delays rendering.
**Action:** Always load external assets like Google Fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS files to enable parallel downloading.
