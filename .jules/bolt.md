...
## 2025-02-05 - CSS @import Render Blocking
**Learning:** Loading external assets like Google Fonts via `@import` inside a CSS file creates a render-blocking request chain (HTML -> CSS -> Font). The browser cannot start downloading the font until the CSS is fully downloaded and parsed.
**Action:** Always prefer loading external fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>`. This allows the browser to discover and download the font in parallel, significantly improving First Contentful Paint (FCP).
