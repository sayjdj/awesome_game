...
## 2024-05-24 - CSS Import Font Blocking
**Learning:** Using `@import` in CSS files to load external assets like Google Fonts creates a render-blocking request chain, delaying First Contentful Paint.
**Action:** Always prefer loading external assets via `<link rel="preconnect">` and `<link rel="stylesheet">` directly in the HTML `<head>` instead of using `@import` in CSS files.
