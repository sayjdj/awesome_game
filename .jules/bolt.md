...
**Learning:** In legacy frontends, using `@import` inside a CSS file to load external assets like Google Fonts creates a sequential network request chain (HTML -> CSS -> Font). This blocks the critical rendering path and delays First Contentful Paint (FCP).
**Action:** Always prefer loading external fonts via `<link rel="preconnect">` and `<link rel="stylesheet">` tags directly in the HTML `<head>`. This allows the browser's preload scanner to discover and fetch the font in parallel with the CSS.
